...
def rank5_accuracy(preds, labels):
	rank1 = 0
	rank5 = 0
	# loop over the prediction and ground-truth labels
	for (p, gt) in zip(preds, labels):
		# sort the prob by their index in descending order
		# so that the more confident guesses are at the front of the list
		p = np.argsort(p)[::-1]

		# check if the ground-truth label is in the top5 predictions
		if gt in p[:5]:
			rank5 += 1
